...
class TrajectoryPlanner:
    def __init__(self, vel_max_task: float, aceleration_max_task : float,
                 ...
                 initial_position_task : np.array = np.array([0, 0, 0, 0])): 
    ...
    def planning_joint(self, q_initial, q_final, frequency = 50): 
        ...
        distance = q_final - q_initial
        time_in_aceleration = self.vel_max_join / self.aceleration_max_joint
        distance_in_aceleration = self.aceleration_max_joint*(time_in_aceleration**2)/2
        time_in_deceleration = time_in_aceleration
        distance_in_deceleration = self.vel_max_joint*time_in_deceleration + -1*self.aceleration_max_joint*(time_in_deceleration**2)/2
        ...
        slice_aceleration = self.aceleration_max_joint/frequency
        slices = total_time * frequency
        ...
        slices_in_aceleration = time_in_aceleration * frequency
        slices_in_deceleration = time_in_deceleration * frequency
        
        acelerations[:slices_in_aceleration] = slice_aceleration
        acelerations[slices_in_deceleration:] = -1*slice_aceleration
        
        for i in range(1, slices):
            speeds[i] = speeds[i-1] + acelerations[i]
            positions[i] = positions[i-1] + speeds[i]
